...
export async function updateProfileAction(formData: FormData) {
  ...
  if (
    ...
    // First deactivate previous selections so unchecked roles
    // do not remain active in the database.
    const {
      error: deactivateRolesError,
    } = await supabase
      .from("company_roles")
      .update({
        is_active: false,
    ...
    if (deactivateRolesError) {
      redirect(
        message(
          "error",
          deactivateRolesError.message,
        ),
      );
    }

    // Reactivate/upsert only roles currently selected by the user.
    for (
      const roleType of selectedRoles
    ) {
      const details = value(
        formData,
    ...
    // Rebuild the current selection so removed sectors
    // do not remain stored after editing.
    const {
      error: clearSectorsError,
    } = await supabase
