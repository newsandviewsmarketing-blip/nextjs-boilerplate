...
export default async function LaboratoryDetailPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ error?: string; message?: string }> }) { const { slug } = await params; const messages = await searchParams; const lab = await loadLaboratory(slug); if (!lab) notFound(); return <main><SiteHeader />
  <section className="page-hero"><div className="shell company-profile-hero">{lab.logo_url ? <img className="company-profile-logo" src={lab.logo_url} alt={`${lab.laboratory_name} logo`} loading="lazy" decoding="async" /> : <div className="company-mark company-profile-logo">{initials(lab.laboratory_name)}</div>}<div><span className="section-kicker">{lab.is_sample ? "SAMPLE LABORATORY" : "VETCONNECT VERIFIED LABORATORY"}</span><h1>{lab.laboratory_name}</h1><p>{lab.description || lab.laboratory_type}</p><div className="profile-chips"><span>{lab.laboratory_type}</span><span>{lab.city || "Pakistan"}</span>{lab.accreditation_verified && <span>Accreditation verified</span>}</div></div></div></section>
