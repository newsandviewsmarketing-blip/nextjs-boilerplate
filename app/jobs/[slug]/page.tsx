...
async function loadJob(slug: string): Promise<PublicJob | null> {
  ...
  const { data } = await supabase.from("public_jobs").select("id, slug, title, description, sector, city, province, district, tehsil, address, employment_type, minimum_qualification, minimum_experience, deadline, company_user_id, company_id, company_name").eq("slug", slug).maybeSingle();
...
export default async function JobDetailPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ error?: string; message?: string }> }) {
  ...
  return <main><SiteHeader /><section className="page-hero"><div className="shell"><span className="section-kicker">{job.sector || "VETERINARY CAREER"}</span><h1>{job.title}</h1><p>{job.company_name} • {job.city || "Pakistan"} • {job.employment_type}</p><div className="profile-chips">{job.minimum_qualification && <span>{job.minimum_qualification}</span>}<span>{job.minimum_experience}+ years</span>{job.deadline && <span>Deadline: {job.deadline}</span>}</div></div></section>
    <section className="section compact-section"><div className="shell two-col"><div className="backend-form-card"><h2>Opportunity details</h2><p>{job.description}</p><dl className="product-specs"><div><dt>Employer</dt><dd>{job.company_name}</dd></div><div><dt>Location</dt><dd>{[job.city, job.province].filter(Boolean).join(", ") || "Pakistan"}</dd></div><div><dt>Employment type</dt><dd>{job.employment_type}</dd></div><div><dt>Minimum qualification</dt><dd>{job.minimum_qualification || "See job description"}</dd></div><div><dt>Minimum experience</dt><dd>{job.minimum_experience} years</dd></div></dl>{(job.company_id || job.company_user_id) && <Link className="button button-secondary" href={`/companies/${job.company_id || job.company_user_id}`}>View employer profile</Link>}</div>
